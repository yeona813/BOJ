...
### 메모
 ...
 <p>두 소수 차이가 가장 적은 골드바흐 파티션을 구해야하며 답은 첫째값, 둘째값으로 이루어져 있고 첫번째 값을 입력의 절반부터 시작해 1씩 감소하고 첫째값이 소수인지 판별한다. 그리고 나서 둘째값이 소수인지 판별하고 둘다 소수이면 결과를 출력하면 된다. </p>

### 골드바흐의 추측

 <p>정수론의 미해결 문제로, 2보다 큰 모두 짝수는 두개의 소수(prime numbeer)의 합으로 표시할 수 있다.</p>
